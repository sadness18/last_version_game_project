...
public:
	entity(Image& image, float X, float Y, int W, int H, float Dx, float Dy, String Name) //конструктор с параметрами
	{
		...
		movetimer = 0; health = 100; sweem = 0; count_gametime = 0; randomx = 0; movetimer_buff = 0; bubbles = 0; check_bubble_fly = 0;
		check_shark_stun = 0; movetimer_stun = 0; count_get_eat = 0; count_get_buff = 0; count_stun_shark = 0; movetimer_health = 0;
...
bool is_game(RenderWindow& window) //ф-ия is_game, принимающая в качестве параметра ссылку на объект окно из ф-ии main. Там же она и вызывается
{
	...
	while (window.isOpen()) //цикл "пока открыто окно". Все, что в нем написано, происходит с постоянным обновлением (каждый фрейм (тик))
	{
	  ...
	  /*------------------------------------------------------------------------*/
		...
		{
			text.setString(/*L"Здоровье: " + playerhealth.str() + */L"Время игры: " + minutesstring.str() + L" минут(ы) " + secondsstring.str() + L" секунд(ы) "); //выводим на экран
